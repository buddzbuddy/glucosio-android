...
public class DatabaseHandler {
    ...
    public AppSettings getAppSettings(){
        AppSettings settingObj =realm.where(AppSettings.class).findFirst();
        return settingObj;
    }
    public void updateAppSettings(AppSettings newAppSettings) {
        realm.beginTransaction();
        AppSettings oldAppSettings = realm.where(AppSettings.class).equalTo("id", 1).findFirst();
        oldAppSettings.deleteFromRealm();
        newAppSettings.setId(1);
        realm.copyToRealm(newAppSettings);
        realm.commitTransaction();
    }
    ...
    public List<Double> getWeightReadingAsArray() {
        ...
        for (i = 0; i < readings.size(); i++) {
            WeightReading singleReading = readings.get(i);
            double reading = singleReading.getWeight();
            readingArray.add(reading);
        }
    ...
    public void addFoodReading(FoodReading reading) {
        realm.beginTransaction();
        reading.setId(getNextKey("food"));
        realm.copyToRealm(reading);
        realm.commitTransaction();
    }

    public void editFoodReading(long oldId, FoodReading reading) {
        // First delete the old reading
        deleteFoodReading(getFoodReadingById(oldId));
        // then save the new one
        addFoodReading(reading);
    }

    public FoodReading getFoodReadingById(long id) {
        return realm.where(FoodReading.class)
                .equalTo("id", id)
                .findFirst();
    }

    public void deleteFoodReading(FoodReading reading) {
        realm.beginTransaction();
        reading.deleteFromRealm();
        realm.commitTransaction();
    }

    public List<FoodReading> getFoodReadings() {
        RealmResults<FoodReading> results =
                realm.where(FoodReading.class)
                        .sort(RealmField.CREATED.key(), Sort.DESCENDING)
                        .findAll();
    ...
    public List<Long> getFoodIdAsArray() {
        List<FoodReading> readings = getFoodReadings();
        ArrayList<Long> idArray = new ArrayList<>();
        int i;

        for (i = 0; i < readings.size(); i++) {
            long id;
            FoodReading singleReading = readings.get(i);
            id = singleReading.getId();
            idArray.add(id);
        }
    ...
    public List<String> getFoodReadingAsArray() {
        List<FoodReading> readings = getFoodReadings();
        ArrayList<String> readingArray = new ArrayList<>(readings.size());
        int i;

        for (i = 0; i < readings.size(); i++) {
            FoodReading singleReading = readings.get(i);
            String reading = singleReading.getReading();
            readingArray.add(singleReading.getMealTime() + " - " + singleReading.getProductName() + " " + reading + " гр");
        }

        return readingArray;
    }

    public List<String> getFoodColorAsArray() {
        List<FoodReading> readings = getFoodReadings();
        ArrayList<String> readingArray = new ArrayList<>(readings.size());
        int i;
        ...
        for (i = 0; i < readings.size(); i++) {
            FoodReading singleReading = readings.get(i);
            String reading = "";
            if(!singleReading.getColor().isEmpty()){
                reading = singleReading.getColor();
            }
            readingArray.add(reading);
        }
    ...
    public List<String> getFoodReadingDateTimeAsArray() {
        List<FoodReading> readings = getFoodReadings();
        ArrayList<String> datetimeArray = new ArrayList<>();
        int i;
        DateFormat inputFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm");
    ...
    private long getNextKey(String where) {
        ...
        switch (where) {
            ...
            case "cholesterol":
                ...
                break;
            case "food":
                maxId = realm.where(FoodReading.class)
                        .max("id");
                break;
        }
        if (maxId == null) {
